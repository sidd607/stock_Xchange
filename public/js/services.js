stockXchange
    ...
    .factory('NSEStock', function($rootScope, $q, $http, $timeout) {
        var myservice = {};
        myservice._initialized = false;
        myservice.refresh = function() {
            var deferred = $q.defer();
            $http.get('http://spandan.iiitb.ac.in/StockXchange/rest/get_stocks').success(function(data,s) {
              deferred.resolve(data);
                if (myservice._initialized) {
                    $rootScope.$broadcast('updatednseStock', deferred.promise);
                }
    ...
    .factory('NASDAQStock', function($rootScope, $q, $http, $timeout) {
        var myservice = {};
        myservice._initialized = false;
        myservice.refresh = function() {
            var deferred = $q.defer();
            $http.get('http://spandan.iiitb.ac.in/StockXchange/rest/get_stocks_nasdaq').success(function(data,s) {
              deferred.resolve(data);
                if (myservice._initialized) {
                    $rootScope.$broadcast('updatednasdaqStock', deferred.promise);
                }
                myservice._initialized = true;
              });
            myservice.listPromise = deferred.promise;
